...
passport.use(
  new CasStrategy(
    {
      version: "CAS3.0",
      ssoBaseURL: "https://cas.univ-lemans.fr/cas", // ✅ Corrigé ici
      serverBaseURL: "https://lammi-saes5-01.univ-lemans.fr", // ✅ URL complète
      validateURL: "/serviceValidate",
    },
    async (profile, done) => {
      console.log("🟢 Profil CAS reçu :", profile);
...
export const casLogin = function (req, res, next) { 
  passport.authenticate("cas", function (err, user) {
    ...
    return res.redirect('/dashboard'); // ✅ Redirection correcte
  })(req, res, next);
};

export const casCallback = (req, res, next) => {
  passport.authenticate("cas", (err, user) => {
    if (err) {
      console.error("❌ Erreur d'authentification CAS :", err);
      return next(err);
    }
    ...
    req.logIn(user, (err) => {
      ...
      console.log("🎉 Connexion réussie, redirection vers /dashboard");
      res.redirect("/dashboard"); // ✅ Redirection corrigée
    });
  })(req, res, next);
};
